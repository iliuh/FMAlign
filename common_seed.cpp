...
bool getcommonseed(unsigned int hits, unsigned int *locates, _seqseed *comseed)
{
	...
	for (int i = 0; i < hits; ++i)
	{
		if ((seqsid[i] > 0) && (seqsid[i] != tmpid))//此时建立一条新的seqseed
		{
			...
			comseed[seqsn].locLIST = (unsigned int*)malloc(sizeof(unsigned int) * (tmpfreq + 1));

			comseed[seqsn].freqonSEQ = tmpfreq + 1;
			comseed[seqsn].locLIST[tmpfreq] = locates[i] - seqsrank[seqsid[i] - 1];
			tmpfreq++;
		...
		else if ((seqsid[i] > 0) && (seqsid[i] == tmpid))
		{
			...
			comseed[seqsn].locLIST = (unsigned int*)realloc(comseed[seqsn].locLIST, 
...
uint32_t fileter_threshold(uint32_t *freq, uint32_t seednum)
{
	...
	size_t now = 0;
	size_t index;
	if (tmp_freq_count[0].tmp_req == 0) total = seednum - tmp_freq_count[0].tmp_count;
...
_seed *search_from_bwt_more_than_3(char *read)
{
	...
	// vector<uint32_t> hit_seq(seqset[minseqID].seq_size - K + 1, 0);
	...
	for (uint32_t i = 0; i < length_read - K + 1; ++i)
	{
		...
		if (ff)//这个种子出现在所有序列上
		{
			...
			{
				hit_seq[tmp_loc[k]] = number_of_hits/njob;
			}
	...
	bool flag;
	threshold = fileter_threshold(hit_seq, length_read - K + 1);
